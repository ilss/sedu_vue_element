  ...
  <div>
    <div class="dropTarget">
      <div v-if="filesArr.length>0">
        <div v-for="(item) in filesArr" :key="item.name">
          <p>{{content.name}}：{{item.name}}</p>
          <div>{{content.progress}}：<div class="vProgress"><el-progress :stroke-width="18" :text-inside="true" :percentage="progressFn"></el-progress></div></div>
          ...
          <p>
            <el-button type="primary" size="small" v-if="uploadState==0" @click="upload()">{{content.aUpload}}</el-button>
            <el-button type="warning" size="small" v-if="uploadState==1" @click="pauseUpload(item)">{{content.aPause}}</el-button>
            <el-button type="primary" size="small" v-if="uploadState==2"  @click="resume(item)">{{content.aContinue}}</el-button>
            <el-button type="success" size="small" v-if="uploadState==4" @click="retry(item)">{{content.aRetry}}</el-button>
            <el-button type="danger" size="small" v-if="uploadState!=1" @click="deleteFile(item)">{{content.aDelete}}</el-button>
      ...
      <div ref="uploadDropTarget" :class="{ hide: filesArr.length!=0 }">
        <span>{{content.msg.tip}}</span>
        <input class="file-hide" type="file" ref="uploadBrowseButton">
      </div>
    </div>
  </div>
</template>
